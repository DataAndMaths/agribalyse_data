...
#-------------------------------------------------------------------------#
    ...
    #-----------------------------------#
    if type_cont_cat=="Box plot":
        var_cont1_box = st.selectbox("Sélectionner votre variable continue", 
                                     synthese_dataset.select_dtypes(float).columns)
        var_cat2_box = st.selectbox("Sélectionner votre variable catégorielle", 
                                    synthese_dataset.select_dtypes(object).columns)
        if var_cont1_box is not None:
            if var_cat2_box is not None:
                fig = px.box(synthese_dataset, x=var_cat2_box, y=var_cont1_box, 
                             color=var_cat2_box,
                             title="{} en fonction de {}".format(var_cont1_box, var_cat2_box))
                fig.update_layout(boxgap=0, showlegend=False)
                fig.update_xaxes(tickangle=45)
    ...
    #-----------------------------------#
    elif type_cat=="Ridgeline":
        var_cont1_ridge = st.selectbox("Sélectionner votre variable continue", 
                                       synthese_dataset.select_dtypes(float).columns, 
                                       key="cont1_ridge")
        var_cat2_ridge = st.selectbox("Sélectionner votre variable continue", 
                                      synthese_dataset.select_dtypes(object).columns, 
                                      key="cat2_ridge")
        if var_cont1_ridge is not None:
            if var_cat2_ridge is not None:
                fig =  px.violin(synthese_dataset, 
                                 x=var_cont1_ridge, y=var_cat2_ridge,
                                 orientation='h', color=var_cat2_ridge,
